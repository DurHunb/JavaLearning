...
> 当一个事务的内部，还有一个部分需要一个完整的结构进行描述，而这个内部的完整的结构又只为外部事物提供服务，那么整个内部的完整结构最好使用内部类。比如大脑对于人，大脑是内部类
...
> 在Java中，允许一个类的定义位于另一个类的内部，前者称为<font color='red'>内部类</font>，后者成为<font color='red'>外部类</font>
...
- 成员内部类（静态、非静态）：
	- 作为外部类的成员：
		- 调用外部类的结构（静态内部类只可以调用静态结构）
		- <font color='red'>可以被`static`修饰</font>
		- 可以被四种权限修饰
	- 作为一个类;
		- 类内可以定义属性、方法、构造器等
		- 可以被继承
		- 可以用`final`修饰，可以用`abstract`修饰
...
class Person{
    ...
    class Brain{
        ...
        //调用外部类的非静态结构，实际为Person.this.eat();
        eat();
        }
    ...
    static class Nose{
        ...
        public void displat(String name){
            ...
            //2.调用外部类的非静态结构          
            System.out.pringln(name);//方法的形参
            System.out.pringln(this.name);//内部类属性
            System.out.pringln(Person.this.name);//外部类属性
        }
    }
...
> 在局部类中，如果调用局部类所声明的方法（比如`method`）中的局部变量（比如：`num`）的话
>
> 要求此局部变量为`final`
>
> - JDK7及之前，要求此变量显式声明为`final`
> - JDK8及以后，可以省略`final`的声明
...
public class InnerClassTest{
    ...
    public void method(){
        ...
        class AA{
            public void show(){
                //即使在方法内部，没有声明final会报错
                //num = 20;
                
                System.out.println(num);//这个num要求是个final
            }
        }
        
        //亦或者是移动开发场景，是匿名实现类的匿名对象，num要求是final
...
<img src=".\面向对象.assets\image-20201130152415941.png" alt="image-20201130153954147" style="zoom:50%;" />
...
- `final`用来修饰变量：此时变量称为一个常量
  - 修饰属性：
    - 对象(引用数据类型)赋值以后，内容可变，引用不可变
    - 基本数据类型赋值以后，只能在方法体内部调用，不能重新赋值
  - 修饰局部变量
...
- `abstract`修饰方法：抽象方法
  ...
  - 父类存在抽象方法，子类必须**重写**父类的抽象方法

    父类存在抽象方法，子类为抽象类，则可以不重写——**有抽象方法必不能被调用**
...
abstract class Person{
    ...
    public abstract String show(int i);
}
...
- JDK8，除了定义全局常量和抽象方法外，还可以定义静态方法、默认方法
  ...
  - 静态方法：`public static void`
  	
  	- 接口中定义的静态方法，只能通过接口自身去调
  - 默认方法：`public default void`，书写时可以省略`public`
  	- 通过实现类的对象，可以调用接口的默认方法,
  	
  	- 在子类(实现类中)中调用接口中的默认方法
...
- 如果**子类（实现类）继承的父类**，和**实现的接口**中有<font color='red'>同名同参数</font>的方法。

  ​	  那么在子类没有重写该方法时，子类对象默认调用的是父类中同名同参数的方法——<font color='red'>类优先原则</font>

  ​	——注，如果是父类和接口中有同名<font color='red'>属性</font>，那么子类的对象调用时会报错

- 如果**实现类**实现了**多个接口**，而这**多个接口**中定义了<font color='red'>同名同参数</font>的方法

  ​	 那么在实现类没有重写此方法的情况下，会报错——<font color='red'>接口冲突</font>
...
public interface CompareA{
    ...
    public default void method2(){
        Systen.out.pringln("Compare上海");
    }
    //默认方法的public可以省略
    //是省略，而不是权限变成默认
    default void method3(){
        System.out.println("Xompare");
    }
}

class Subclass implements CompareA{
    //因为没有抽象方法，所以可以不需要填入任何东西
} 

//如何在子类(实现类中)中调用父类、接口中的方法
...
虽然是`new`接口，其实是它的实现类的对象

```java
public class InterfaceSituationTest{
    //USB是一个接口，Flash是它的实现类
    public static void main(String[] args){
...
```java    
    ...
    public void test3(){
        ...
        String s1 = Integer.toString(in2);
    }
...
<img src=".\面向对象.assets\jvm.png" alt="image-20210422205658860" style="zoom: 80%;" />
...
<img src=".\面向对象.assets\MVC.png" alt="image-20210422205658860" style="zoom: 120%;" />
...
class Order{
    ...
    public static synchronized Order getInstance(){
        //方式一:效率稍差，
        //当多个线程时，线程会卡在前面等待，然而当已经存在单例时，并不需要等待，直接返回单例
