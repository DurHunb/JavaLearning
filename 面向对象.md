...
1、接口中定义的静态方法，只能通过接口去调

2、通过实现类的对象，可以调用接口的默认方法

3、如果实现类重写了接口中的默认方法，调用时，调用的是重写后的方法

4、如果子类（实现类）继承的父类，和实现的接口中有<font color='red'>同名同参数</font>的方法。

​	  那么在子类没有重写该方法时，子类对象默认调用的是父类中同名同参数的方法——<font color='red'>类优先原则</font>

​	——注，如果是父类和接口中有同名<font color='red'>属性</font>，那么子类的对象调用时会报错

5、如果实现类实现了多个接口，而这多个接口中定义了<font color='red'>同名同参数</font>的方法

​	 那么在实现类没有重写此方法的情况下，会报错——<font color='red'>接口冲突</font>

6、在子类(实现类中)中调用接口中的默认方法
